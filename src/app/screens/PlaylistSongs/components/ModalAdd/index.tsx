...
export const ModalAdd: FC<IProps> = ({
  isVisible,
  onAdd,
  onClose,
  songs,
  ...
  const _onChange = (input: NativeSyntheticEvent<TextInputChangeEventData>) => {
    ...
    if (textSearch.length) {
      newSongsVisibles = songs.filter(song => {
        return (
          song.title.toLowerCase().indexOf(textSearch.toLowerCase()) !== -1
        );
      });
    }
  ...
  const SongCard: FC<{song: MSong}> = ({song}) => {
    ...
    return (
      <Ripple onPress={_onChange}>
        <View
          style={[staticStyles.songCardContainer, dynamicStyles.borderColor]}>
          <View style={staticStyles.songLeft}>
            {song.title.length > 40 ? (
              <AutoScrolling>
                <Text style={dynamicStyles.texColor}>{song.title}</Text>
              </AutoScrolling>
            ) : (
              <Text style={dynamicStyles.texColor}>{song.title}</Text>
            )}
